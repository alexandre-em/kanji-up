...
export default class recognitionService {
  ...
  constructor() {
    this.baseUrl = axios.create({
      baseURL: `${process.env.REACT_APP_KANJI_BASE_URL}/recognition`,
  ...
  getRecognitions(
    ...
    return this.baseUrl.get(
      `/all?page=${page}&limit=${limit}&query=${encodeURI(query)}`,
      options
    );
  }

  userValidation(isValid: boolean, recognitionId: string) {
    return this.baseUrl.patch(`/validation/${recognitionId}`, {
      is_valid: isValid,
    });
  }
