...
export default function GradientCard({ buttonTitle, image, onPress, title, subtitle, disabled = false }: GradientCardProps) {
  const Gradient = Platform.select({ web: View, native: LinearGradient as any });
  const webGradient =
    Platform.OS === 'web' ? [{ background: 'linear-gradient(200deg, rgba(255,255,255,0) 0%, rgba(255,255,255,1) 85%)' }] : [];
  ...
  return (
    <View style={styles.card}>
      <ImageBackground
        source={image}
        style={{ flex: 1 }}
        imageStyle={{ height: 220, width: 'auto', borderRadius: 15, opacity: 0.65 }}
        resizeMode="cover">
