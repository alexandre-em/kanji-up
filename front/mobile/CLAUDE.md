...
- Couleurs : toujours les tokens RNUI (`Colors.$backgroundPrimaryHeavy`, `$textDefault`…), jamais
  ...
  `Canvas`** (`src/components/canvas.tsx`) au moment de la capture. Pendant que l'utilisateur
  dessine, le canvas suit normalement le thème (fond et trait adaptés au clair/sombre). Mais
  l'image capturée (prop `forceCaptureColors`, activée juste avant l'appel à `ViewShot.capture()`
  puis désactivée juste après) force **toujours** un fond blanc et un trait noir, quel que soit le
  thème actif — ce n'est pas un oubli : cette image part telle quelle vers le modèle de
  reconnaissance, qui n'a été entraîné que sur ce pattern (fond blanc, trait noir), et c'est aussi
  ce qui est sauvegardé/affiché ensuite (résultats, review). La laisser suivre le thème casserait
  la reconnaissance en mode sombre.
