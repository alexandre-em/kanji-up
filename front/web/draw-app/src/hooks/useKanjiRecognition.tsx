...
import { MODEL_INPUT_WIDTH, MODEL_INPUT_HEIGHT, MIN_CONFIDENCE, labels } from '@/constants';
...
export default function useKanjiRecognition() {
  const startDb = useIndexedDb();
  const [loading, setLoading] = useState<boolean>(false);
  const [model, setModel] = useState<tflite.TFLiteModel>();

  const downloadThenSave = useCallback(
    async (onProgress: (progress: number) => void, onFinishDownload?: () => void, headers?: AxiosRequestHeaders) => {
      const modelUrl = process.env.KANJI_MODEL_DL_URL || '';
      const buffer: AxiosResponse<ArrayBuffer> = await axios.get(modelUrl, {
