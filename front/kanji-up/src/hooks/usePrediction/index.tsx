...
export default function usePrediction() {
  ...
  const downloadThenSave = useCallback(
    async (
      ...
      const url = (
        await axios.get(
          `https://kanjiup1-alexandreemem.b4a.run/recognition/model?model=${kanjiPredictionConstants.MODEL_KEY_DL}`,
          { headers }
        )
      ).data.web;
      const buffer: AxiosResponse<ArrayBuffer> = await axios.get(url, {
  ...
  const isBufferStored = React.useMemo(
    async () => JSON.parse(localStorage.getItem("kanjiPrediction") || "false"),
    []
  );
  ...
  const loadModel = useCallback(async () => {
    ...
    if (await isBufferStored) {
      ...
      const loadedModel: tflite.TFLiteModel = await tflite.loadTFLiteModel(
        buffer,
        options
      );
  ...
  const predict = useCallback(
    async (image: string) => {
      ...
      try {
        ...
        // image preprocessing
        const { MODEL_INPUT_WIDTH, MODEL_INPUT_HEIGHT, MIN_CONFIDENCE } =
          kanjiPredictionConstants;
        const imageTensor: tf.Tensor3D = tf.browser.fromPixels(img, 1);
        const resizedImage: tf.Tensor3D = tf.image.resizeBilinear(imageTensor, [
          MODEL_INPUT_WIDTH,
          MODEL_INPUT_HEIGHT,
        ]);
        // prediction of the model on the image
        const prediction: tf.Tensor = model!.predict(
          tf.expandDims(tf.div(resizedImage, 255), 0)
        ) as tf.Tensor;
        ...
        const indexes = predictionArray[0]
          ...
          .filter(
            (iconfidence: number) =>
              predictionArray[0][iconfidence] >= MIN_CONFIDENCE
          );
