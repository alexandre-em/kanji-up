...
export default function usePrediction() {
  ...
  const isBufferStored = React.useMemo(async () => {
    ...
    const keyOccurences = keys.filter((k) =>
      k.includes("kanjiPrediction")
    ).length;
  ...
  const loadModel = useCallback(async () => {
    ...
    if (await isBufferStored) {
      ...
      const asyncStorageKeyOccurences = asyncStorageKeys.filter((k) =>
        k.includes("kanjiPrediction")
      ).length;
      ...
      const loadedModel: tf.GraphModel<tf.io.IOHandler> =
        await tf.loadGraphModel(
          asyncStorageIO(kanjiPredictionConstants.MODEL_KEY) as IOHandler
        );
