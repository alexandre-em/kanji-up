...
const addData = async (kanjiDetail: any, progressBar: cliProgress.SingleBar) => {
  try {
    const kanjiPromises: Promise<any>[] = [];
    kanjiPromises.push(
    ...
    const examples = kanjiDetail.examples.map((example: { japanese: string; meaning: { english: string } }) => ({ japanese: example.japanese, meaning: example.meaning.english }));
