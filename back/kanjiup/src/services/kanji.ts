...
export const searchCharacter = async (query: string, page: number = 1, limit: number = 10) => {
  ...
  const q: Partial<{ deleted_at: string | null, kanji: { $in: any } | null }> = { deleted_at: null };
  q['kanji'] = {
  ...
  return KanjiModel
    .paginate(q, { limit, page, populate, select: '-_id -__v -examples' })
};
